...
- Fix compilation warning (GH-125).
...
- Zone distribution and zone record distribution (GH-115)
...
- Configuration setting for base URL (GH-113)
- Let's Encrypt certificate methods (GH-118)
...
- Removed premium_price attribute from registrar order responses (GH-120). Please do not rely on that attribute, as it returned an incorrect value. The attribute is going to be removed, and the API now returns a null value.
...
- Update registrar endpoint URLs (GH-112)

### Fixed

- Relax poison dependency to fix incompatibility with Phoenix (GH-110)
...
- Support for DNSSEC endpoints (GH-109)
...
- `unicode_name` to `Dnsimple.Domain` struct (GH-104)

### Changed

- Improve and unify documentation styles (GH-101)

### Fixed

- Fix compilation warnings on Elixir 1.4.0 (GH-106)
...
- Support for get domain premium price endpoint (GH-87)
- Request logging (GH-81)
- Support for collaborator endpoints (GH-74)
- Support to zone records regions (GH-72)
- New attributes to TLDs (GH-69)

### Changed

- Removed function aliases for simplicity (GH-95)
- Updated structs to reflect latest APIv2 payloads (GH-94)
- Updated registration, transfer, renewal response payload (dnsimple/dnsimple-developer#111, #88)
- Being able to provide settings when applying a service (GH-77)
- Updated httpoison to 0.10.0 (GH-75)
...
- Increased timeout to 30 seconds (GH-67)
...
- Support for pushes (GH-65)
- Support for vanity name server endpoints (GH-60)
- Support to get zone files (GH-58)
- Support for email forward endpoints (GH-56)
- Support for delegation endpoints (GH-55)
- Complete support of service endpoints (GH-53)
- Support for template endpoints (GH-51)
- Support for TLD endpoints (GH-49)

### Changed

- Added aliases and consolidate function naming (GH-47)
...
- Function to start OTP app (GH-45)
- Support for contact endpoints (GH-38)
- Reset domain token endpoint support (GH-37)
- Support for accounts (GH-29)
- Support for filtering and sorting (GH-19)
- Support for domain services (GH-24)
- Support for certificates (GH-26)
- Support for webhooks (GH-27)
- Support for OAuth dance (GH-12)
- Support for whois privacy (GH-6)
- Support for domain auto renewal (GH-5)

### Changed

- Setting a custom user-agent no longer overrides the original user-agent (GH-15)
- Renamed Record struct to ZoneRecord (GH-18).

### Removed

- Removed support for wildcard accounts (GH-16).
