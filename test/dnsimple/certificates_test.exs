defmodule Dnsimple.CertificatesTest do
  ...
  describe ".get_certificate" do
    test "returns the certificate in a Dnsimple.Response" do
      url = "#{@client.base_url}/v2/1010/domains/example.com/certificates/22289"

      use_cassette :stub, ExvcrUtils.response_fixture("getCertificate/success.http", method: "get", url: url) do
        {:ok, response} = @module.get_certificate(@client, "1010", "example.com", "22289")
        assert response.__struct__ == Dnsimple.Response
    ...
    test "can be called using the alias .certificate" do
      url = "#{@client.base_url}/v2/1010/domains/example.com/certificates/22289"
