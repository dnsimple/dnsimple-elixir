...
Add new entries to the `## Unreleased` section at the top of `CHANGELOG.md`. When a change requires a major version, prefix its entry with `**BREAKING**:`, for example `- **BREAKING**: Drop support for Elixir or OTP < X.Y`. The release process uses this prefix to select the next version.
