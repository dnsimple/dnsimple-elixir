...
1. **Determine the new version** using [Semantic Versioning](https://semver.org/)
   ...
   Check the `## Unreleased` section in `CHANGELOG.md` for entries prefixed with `**BREAKING**:`. Any such entry requires a **MAJOR** version, whatever the other entries are. Raising the minimum supported Elixir or OTP version is always a breaking change, so its changelog entry must carry this prefix.
